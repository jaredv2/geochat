...
def callback():
    ...
    if not q1("SELECT id FROM oauth_states WHERE state=%s", (state,)):
        return _auth_error("Invalid login state.",
                           "This can happen if you opened multiple login tabs. Please try again.")
    ...
    if not tr.ok:
        return _auth_error("Discord login failed.",
                           f"Discord returned {tr.status_code}. Please try again.")
